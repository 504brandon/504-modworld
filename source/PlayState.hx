...
class PlayState extends MusicBeatState {
	...
	public static var theFunneNumber:Float = 1;
		...
		if (!PlayStateChangeables.Optimize) {
			switch (stageCheck) {
				...
				case 'ryanHouse':
					{
						...
						var bg:FlxSprite = new FlxSprite(-700, -100).loadGraphic(Paths.image('ryan/ryanHouse'));
						bg.setGraphicSize(Std.int(bg.width * 1.5));
						bg.setGraphicSize(Std.int(bg.height * 1.6));
						bg.antialiasing = true;
						bg.scrollFactor.set(.98, 0.9375);
						bg.active = false;
						bg.visible = true;
						add(bg);
					}
				...
				case 'ballisticAlley':
					...
					bg.frames = Paths.getSparrowAtlas('whitty/BallisticBackground');
					bg.animation.addByPrefix('gameButMove', 'Background Whitty Moving', 16, true);
				...
				case 'tank':
							{
									defaultCamZoom = 0.9;
									curStage = 'tank';
									var sky:FlxSprite = new FlxSprite(-400,-400).loadGraphic(Paths.image('tank/tankSky'));
									...
									var buildings:FlxSprite = new FlxSprite(-200,0).loadGraphic(Paths.image('tank/tankBuildings'));
				...
				case 'fleet':
					...
					var bgsonic = new FlxSprite(-254.4, 20).loadGraphic(Paths.image('chaos nightmare/sonic'));
	...
	override function stepHit() {
		...
		if (dad.curCharacter == 'garcelloghosty' && SONG.song.toLowerCase() == 'fading') {
			...
			if (SONG.song.toLowerCase() == 'beatdown-bop') {
				switch curStep {
					case 11:
			...
			if (SONG.song.toLowerCase() == 'kingin-it') {
				switch curStep {
					case 50:
						dad.playAnim('cough', true);
					case 115:
				...
				if (SONG.song.toLowerCase() == 'the-next-video') {
					switch curStep {
						case 376:
							dad.playAnim('seeya', true);
					}
				if (SONG.song.toLowerCase() == 'phantasm')
					switch curStep{
						case 386:
							changeBFCharacter('bf-fleetway-p');
							trace('hello');
						case 640:
							changeBFCharacter('bf-sonic-p');
						case 767:
							changeBFCharacter('bf-fleetway-p');
						case 1023:
							changeBFCharacter('bf-sonic-p');
